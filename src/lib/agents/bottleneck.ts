...
const getVolatilityContribution = createTool({
  ...
  execute: async (input) => {
    ...
    const allTickers = [...new Set([...positions.map((p) => p.ticker), input.ticker])];
      ...
      allTickers.map(async (ticker) => {
        const ac =
          ticker === input.ticker
            ...
            : (positions.find((p) => p.ticker === ticker)?.assetClass as AssetClass) ??
              assetClassForTicker(ticker);
    ...
    const targetVol =
      targetReturns.length > 1 ? ss.sampleStandardDeviation(targetReturns) * 100 : 0;
    ...
    const portfolioVol =
      portfolioReturns.length > 1 ? ss.sampleStandardDeviation(portfolioReturns) * 100 : 0;
    ...
    const targetWeight = positions.find((p) => p.ticker === input.ticker)?.weight ?? 0;
    const marginalContribution = targetWeight * beta * (portfolioVol / 100) * 100;
    const componentVarPct = portfolioVol > 0 ? (marginalContribution / portfolioVol) * 100 : 0;
...
const searchMarketDocuments = createTool({
  ...
  description:
    "Search recent market news and reports relevant to a query. Returns limited results - full RAG integration arrives in Phase 4.",
  inputSchema: z.object({
    query: z.string().describe("Search query, e.g. Tesla earnings or Bitcoin ETF"),
  ...
  execute: async () => {
    // Phase 4 will wire pgvector + NewsAPI for real RAG
