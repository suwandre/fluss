...
const ALTERNATIVE_UNIVERSE: Record<string, { ticker: string; name: string; assetClass: string }[]> = {
  equity: [
    { ticker: "VTI", name: "Vanguard Total Stock Market ETF", assetClass: "etf" },
    ...
    { ticker: "SCHD", name: "Schwab US Dividend Equity ETF", assetClass: "etf" },
...
const getAlternativeAssets = createTool({
  ...
  inputSchema: z.object({
    assetClass: z.string().describe("Asset class to find alternatives for: equity, crypto, bond, etf"),
    excludeTickers: z.array(z.string()).optional().describe("Tickers already in portfolio to exclude"),
    maxResults: z.number().optional().describe("Maximum number of alternatives to return").default(5),
  }),
  ...
  execute: async (input) => {
    ...
    const candidates = ALTERNATIVE_UNIVERSE[input.assetClass] ?? ALTERNATIVE_UNIVERSE["equity"];
    const filtered = candidates.filter((c) => !exclude.has(c.ticker)).slice(0, max);
...
const simulateRebalance = createTool({
  ...
  inputSchema: z.object({
    proposedChanges: z.array(
      z.object({
        action: z.enum(["reduce", "increase", "replace", "add", "remove"]),
        ticker: z.string(),
        targetWeightPct: z.number().describe("Target portfolio weight as percentage (0-100)"),
      }),
    ).describe("List of proposed changes to simulate"),
    benchmarkDays: z.number().optional().describe("Days of historical data for simulation").default(90),
  }),
  outputSchema: z.object({
    current: z.object({
      totalValue: z.number(),
    ...
    projectedPnlPct: z.number().describe("Projected change in portfolio value over benchmark period"),
    projectedVolatilityPct: z.number().describe("Estimated annualized volatility of proposed portfolio"),
  ...
  execute: async (input) => {
    ...
    const priceMap = rows.length > 0
      ? await getBatchPrices(rows.map((r) => ({ ticker: r.ticker, assetClass: r.assetClass })))
      : new Map();

    // Build current portfolio snapshot
    let currentTotalValue = 0;
    const currentPositions: { ticker: string; value: number; assetClass: string }[] = [];
    for (const row of rows) {
      const price = priceMap.get(row.ticker)?.price ?? 0;
      const value = price * row.quantity;
      currentTotalValue += value;
      currentPositions.push({ ticker: row.ticker, value, assetClass: row.assetClass });
    }

    const currentAllocations = currentPositions.map((p) => ({
      ticker: p.ticker,
      weightPct: currentTotalValue > 0 ? Math.round((p.value / currentTotalValue) * 10000) / 100 : 0,
    ...
    for (const pos of currentPositions) {
      const currentWeight = currentTotalValue > 0 ? (pos.value / currentTotalValue) * 100 : 0;
    ...
    for (const change of input.proposedChanges) {
      proposedAssetClasses.set(change.ticker, assetClassForTicker(change.ticker));
    ...
    const totalWeight = [...proposedWeights.values()].reduce((s, w) => s + w, 0);
    ...
    const proposedAllocations = [...normalizedWeights.entries()].map(([ticker, weightPct]) => ({
      ticker,
      weightPct,
    ...
    if (proposedTickers.length > 0) {
      await Promise.all(
        proposedTickers.map(async (ticker) => {
          const ac = proposedAssetClasses.get(ticker) ?? assetClassForTicker(ticker);
          const history = await getHistory(ticker, ac as AssetClass, { days: benchDays });
          const closes: number[] = [];
    ...
    if (totalWeight > 0 && Math.abs(totalWeight - 100) > 1) {
      notes.push(`Weights were normalized from ${Math.round(totalWeight)}% to 100%.`);
    }
    if (minLen < 30) {
      notes.push(`Limited historical data (${minLen} days). Estimates may be unreliable.`);
    }
...
const getRebalanceHistory = createTool({
  ...
  inputSchema: z.object({
    limit: z.number().optional().describe("Max number of past proposals to return").default(5),
  }),
  outputSchema: z.object({
    history: z.array(
      z.object({
