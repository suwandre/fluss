...
export function AgentStep({
  ...
  const dotStatus = status === "error" ? "critical" : status === "done" ? "nominal" : "warning"
    ...
    <div data-slot="agent-step" className="flex gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {status === "running" && isStreaming ? "Streaming…" : STATUS_LABEL_MAP[status]}
          </span>
        ...
        {showStructuredOutput && structuredOutput && (
          <div className="mt-1.5 space-y-0.5">
            {Object.entries(structuredOutput).map(([key, value]) => (
              <div key={key} className="flex gap-1.5 text-[12px] font-mono leading-snug">
          ...
          <Collapsible open={reasoningOpen} onOpenChange={setReasoningOpen}>
            ...
            <CollapsibleContent>
              <div className="mt-1.5 rounded bg-bg-elevated border-l-2 border-border-bright pl-3 pr-2 py-2 text-[13px] text-text leading-relaxed font-sans">
                {reasoning}
                {isStreaming && (
                  <span className="inline-block w-[2px] h-[14px] bg-amber ml-0.5 align-middle animate-cursor-blink" />
                )}
              </div>
            </CollapsibleContent>
          </Collapsible>
        ...
        {isStreaming && !reasoning && (
          <div className="mt-1.5 flex items-center gap-1.5">
            <span className="inline-block w-[2px] h-[14px] bg-amber animate-cursor-blink" />
            <span className="text-[11px] text-text-dim font-mono">streaming…</span>
          </div>
        )}
      </div>
    </div>
